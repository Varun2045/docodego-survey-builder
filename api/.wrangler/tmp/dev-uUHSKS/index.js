...
// .wrangler/tmp/bundle-ink2gF/checked-fetch.js
...
app.put("/api/surveys/:id", async (c) => {
  ...
  try {
    const { meta } = await c.env.DB.prepare(
      "UPDATE surveys SET title = ?, primary_color = ?, logo_url = ? WHERE id = ? AND user_id = ?"
    ).bind(
      title.trim(),
      primary_color?.trim() || null,
      logo_url?.trim() || null,
...
app.put("/api/surveys/:id/questions", async (c) => {
  ...
  try {
    ...
    for (const q of questions) {
      ...
      statements.push(
        c.env.DB.prepare(
          "INSERT INTO questions (id, survey_id, type, text, order_index, options) VALUES (?, ?, ?, ?, ?, ?)"
        ).bind(
          qId,
          surveyId,
          q.type,
...
// .wrangler/tmp/bundle-ink2gF/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ink2gF/middleware-loader.entry.ts
